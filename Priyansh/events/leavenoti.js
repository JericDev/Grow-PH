...
module.exports.onLoad = function () {
	...
	const path = join(__dirname, "cache", "leaveGif", "leave.gif");
	if (existsSync(path)) mkdirSync(path, { recursive: true });	

	const path2 = join(__dirname, "cache", "leaveGif", "leave.gif");
...
module.exports.run = async function({ api, event, Users, Threads }) {
  ...
  const hours = moment.tz("Asia/Kolkata").format("HH");
	...
	const path = join(__dirname, "events", "leave.gif");
	const pathGif = join(path, `${threadID}leave.gif`);
	var msg, formPush
